...
当前发布基线为 **`0.1.0-alpha.4`**；`alpha5-about-update` 分支已完成关于与更新首个切片，但 Alpha 5 仍在开发中，尚未达到发布门禁。语音链路不依赖互联网，只有用户手动检查更新时才会访问 GitHub；核心链路仍属于早期公开测试阶段，多机真机验收还没做完。
