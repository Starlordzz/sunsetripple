...
源码根目录是 `app/src/main/kotlin`，包根 `host.msknet.sunsetripple`。共 58 个 Kotlin 源文件。
...
| `ui` | 全部 Compose 界面、主题品牌、权限策略对象、纯流转决策对象 |
...
界面文件：`HomeScreen`、`ScanScreen`、`BluetoothScanScreen`、`NearbyScanScreen`、`RoomScreen`、`LoopbackScreen`。品牌与动效在 `SunsetRippleTheme`、`BrandElements`、`SunsetControls`、`SunsetMotion`。
...
- 没有 `strings.xml` / `colors.xml`，没有多语言。
- 没有深色配色方案，只有 Material 3 `lightColorScheme`。
