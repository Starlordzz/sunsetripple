...
| `ios/` | iOS Flutter 宿主、PlatformAudioPlugin (AudioUnit VoiceProcessingIO) 与 BleL2capPlugin |
...
## 近场文字聊天室 (Room Text Chat)

采用**纯内存、仅限当前房间生命周期、零服务端**的设计，WiFi 房与蓝牙房共享同一套 Dart 会话模型与 UI 抽屉面板。

### 1. 通道隔离与路由
- **WiFi 房**：文字消息经 `ChatMessagePayload` 编码为 `FrameType.chat (0x0c)`，严格路由至 TCP 8988 控制信道，由房主向其他客户端中继转发；**严禁**泄漏至 UDP 8989 音频端口，保障语音最高调度优先级。
- **蓝牙房**：复用现有 BLE L2CAP 逻辑链路信道，Dart 层推送到 `sendL2capData`，Android 原生层排他性广播分发。
- **安全信封**：按当前产品要求默认明文；仅在会话配置 `secureCodec` 时，`sendFrame` 才密封为 `FrameType.sealed (0x0b)`。

### 2. 平台能力现状与已知缺口 (Platform Matrix)

| 平台 | WiFi 文字聊天 | 蓝牙 (BLE L2CAP) 文字聊天 | 现状与已知阻塞缺口 |
| :--- | :--- | :--- | :--- |
| **Android** | ✅ 完全支持 | ✅ 完全支持 | WiFi Direct P2P 建组与 Client 均已验证；`BleL2capPlugin.kt` 提供 1024B 原生缓冲与主机多路广播。 |
| **iOS** | ⚠️ 局域网支持 | ❌ 有阻塞缺口 | 仅支持同 WiFi/热点下的 LAN TCP 聊天；`BleL2capPlugin.swift` 目前**缺少流分片重组与主机广播中继**。 |
| **HarmonyOS** | ❌ 未通数据面 | ❌ 未通数据面 | `harmonyos/` 仅具备基础 UDP 发现（`LanRoomDiscovery.ets`），尚未打通房间数据信道。 |
