...
当前 Flutter 实现使用 `HostTransferCodec` v2：每个成员附带 16 字节
`sessionToken`，新房主可在重连时恢复原成员号；格式见
[协议规范](协议规范.md#host_transfer--host_snapshot)。没有完整 token 的计划会自动回退到
兼容旧客户端的 v1，成员随后通过普通 JOIN 重新分配身份。解码会拒绝版本号错误、成员数越界、
端点含非 ASCII、UTF-8 非法、token 不完整和多余尾字节。
